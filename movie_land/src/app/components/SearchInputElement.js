...
import fetchSearch from "@/app/services/fetchSearch";
import fetchHome from "@/app/services/fetchHome";

const SearchInputElement = ({searchText, onDataUpdate}) => {
    ...
    async function search() {
        ...
        if (searchValue.length > 3) {
            result = await fetchSearch(searchValue, '');
        } else {
        ...
        <div>
            <form>
                ...
                <div className="relative">
                    ...
                    <input type="search" id="default-search" onChange={(e) => setSearchValue(e.target.value)}
                           onKeyUp={search}
                           className="block w-full p-4 ps-10 text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                           placeholder={`Search ${searchText}`} required/>
